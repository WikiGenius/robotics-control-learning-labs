...
The repository now includes a small public LQR starter lab and links to focused public support repositories, including an inverted-pendulum control lab. Unpublished or paper-specific control experiments still stay private until they can be safely distilled into public examples.
...
- Public link to an inverted-pendulum control lab with PID, LQR, observers, and LQG.
...
- Optional state and control plot when `matplotlib` is available.

Related public artifacts:

- `ipendulum` includes inverted-pendulum demo GIFs and MATLAB simulation runners for PID, LQR/LQG, and observer workflows.
...
- [x] Link public inverted-pendulum control support repo.
- [ ] Add first state-space modeling lab note.
- [ ] Add observer design example.
- [ ] Add a distilled inverted-pendulum note that summarizes the public `ipendulum` examples.
